...
public class TraineeDaoImpl implements TraineeDao {
    ...
    public void deleteByUsername(String username) {
        entityManager.createNativeQuery("DELETE FROM trainings WHERE trainee_id IN (SELECT id FROM trainees WHERE username = ?)")
                .setParameter(1, username)
                .executeUpdate();
        entityManager.createNativeQuery("DELETE FROM trainees_trainers WHERE trainee_id IN (SELECT id FROM trainees WHERE username = ?)")
                .setParameter(1, username)
                .executeUpdate();
        entityManager.createNativeQuery("DELETE FROM trainees WHERE username = ?")
                .setParameter(1, username)
                .executeUpdate();
    }
    ...
    public boolean existsByUsername(String username) {
        return entityManager.createQuery("SELECT COUNT(t) FROM Trainee t WHERE t.username = :username", Long.class)
                .setParameter("username", username)
                .getSingleResult() > 0;
    }
    ...
    public List<Training> findTraineeTrainings(String username, LocalDateTime fromDate, LocalDateTime toDate,
        ...
        if (trainerName != null && !trainerName.isEmpty()) {
            predicates.add(cb.like(cb.lower(trainerJoin.get("firstName")), "%" + trainerName.toLowerCase() + "%"));
        }
    ...
    public List<Trainer> findAvailableTrainersForTrainee(String traineeUsername) {
        return entityManager.createQuery(
                        "SELECT tr FROM Trainer tr WHERE tr NOT IN " +
                                "(SELECT t.trainers FROM Trainee t WHERE t.username = :username)",
                        Trainer.class)
